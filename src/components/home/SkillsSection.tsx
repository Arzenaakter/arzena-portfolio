                ...
                <CardContent className="p-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {coreStrengths.map((strength, index) => (
                      <div
                        key={index}
                        className={`text-center p-2 rounded-lg bg-primary/5  hover:scale-105 hover:-translate-y-1 transition-all duration-300 cursor-pointer group/strength ${
                          isVisible ? "animate-fade-in" : "opacity-0"
                        }`}
                        style={{ animationDelay: `${index * 100}ms` }}
                      >
                        <div className="w-full justify-center py-2  group-hover/strength:text-primary transition-all duration-300">
                          {strength}
                        </div>
                      </div>
                    ))}
